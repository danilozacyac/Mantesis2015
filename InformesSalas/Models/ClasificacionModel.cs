...
namespace InformesSalas.Models
{
    public class ClasificacionModel
    {
        ...
        public ObservableCollection<ClasificacionDto> GetClasificacion(ClasificacionDto temaPadre, int tatj)
        {
            ObservableCollection<ClasificacionDto> temas = new ObservableCollection<ClasificacionDto>();

            SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["salasConnection"].ConnectionString);
            SqlCommand cmd;
            SqlDataReader dataReader;

            string sqlCadena = "SELECT * FROM salasClasificaciones WHERE padre = @Padre AND (tpoTesis = @TipoTesis OR tpoTesis = 2) ORDER BY idClasif";
            ...
            try
            {
                ...
                if (temaPadre == null)
                    ...
                    cmd.Parameters.AddWithValue("@Padre", temaPadre.IdClasif);

                cmd.Parameters.AddWithValue("@TipoTesis", tatj);
                ...
                if (dataReader.HasRows)
                {
                    while (dataReader.Read())
                    {
                        ClasificacionDto tema = new ClasificacionDto(temaPadre);
                        tema.IdClasif = Convert.ToInt32(dataReader["idClasif"]);
                        tema.IsSelected = false;
                        tema.Descripcion = dataReader["Descripcion"].ToString();
                        tema.Nivel = Convert.ToInt32(dataReader["nivel"]);
                        tema.Padre = Convert.ToInt32(dataReader["padre"]);
                        tema.Estado = Convert.ToBoolean(dataReader["estado"]);
                        tema.Hijos = this.GetClasificacion(tema, tatj);
